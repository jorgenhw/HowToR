...
Pitch for kvinder når polite, er: 252.895 -15.646. 
Pitch for mænd når polite 252.895 - (- 112.054 -15.646), = -127.622
Interaction betyder, at attitude polite er 5.855 mindre for mænd end for kvinder. Dvs. at mænd taler med "5 hz" mindre end kvinder (korrigeret for køn) når de taler i en polite situation.
...
The *sub* and *gsub* function
```{r}
x <- c("oone", "two", "three", "four", "five")

# Replace all the o's with k's in vector x (could also be whole words)
sub("o", "k", x)
# Beware, that for x[1], it replaces only the first o, not the second one...

# Introducing gsub (global sub) which replaces all the content in a variable (meaning all the o's)
gsub("o", "k", x)
...
# 3 John.Aldag@parl.gc.ca    
# 4 @jwaldag                 
# 5 Omar Alghabra            
# 6 Mississauga Centre 

# And we wanted to make a new one, with only the lines that starts with @:
new <- df[grepl("^@",df$`Liberal Party of Canada`),]

# You can also combine functions. If you want to isolate the value that starts with @ AND ends with K: (starts with = ^, ends with = $)
new <- df[grepl("^@ | K$",df$`Liberal Party of Canada`),]
```
